...
export default function CoinDetailContent({ coin, timeframe, setTimeframe, chartData, chartLoading }) {
  ...
  const athDate = md.ath_date?.usd ? new Date(md.ath_date.usd).toLocaleDateString('fr-FR') : 'N/A';
  const atlDate = md.atl_date?.usd ? new Date(md.atl_date.usd).toLocaleDateString('fr-FR') : 'N/A';
      ...
      <div className="coin-detail-hero">
        ...
        <div className="coin-detail-hero-info">
          <div className="coin-detail-title-row">
            ...
            {coin.market_cap_rank && <span className="coin-detail-rank">#{coin.market_cap_rank}</span>}
          </div>
          {categories.length > 0 && (
            <div className="coin-detail-categories">
              {categories.slice(0, 4).map((cat) => (
                <span key={cat} className="coin-detail-cat-tag">
                  {cat}
                </span>
              ))}
            </div>
        ...
        <div className="coin-detail-stats">
          <div className="coin-detail-stat">
            ...
            {md.market_cap_change_percentage_24h !== null && md.market_cap_change_percentage_24h !== undefined && (
              <span className={`coin-detail-stat-sub ${getChangeClass(md.market_cap_change_percentage_24h)}`}>
                {getChangeSign(md.market_cap_change_percentage_24h)}
                {md.market_cap_change_percentage_24h.toFixed(2)}% (24h)
              </span>
            )}
          </div>
          <div className="coin-detail-stat">
          ...
          <div className="coin-detail-stat">
            ...
            <span className="coin-detail-stat-value">
              {md.fully_diluted_valuation?.usd ? `$${formatLargeNumber(md.fully_diluted_valuation.usd)}` : 'N/A'}
            </span>
          </div>
          <div className="coin-detail-stat">
          ...
          <div className="coin-detail-stat">
            ...
            <span className="coin-detail-stat-value">{md.total_supply ? formatLargeNumber(md.total_supply) : '∞'}</span>
          </div>
          <div className="coin-detail-stat">
            <span className="coin-detail-stat-label">Offre maximale</span>
            <span className="coin-detail-stat-value">{md.max_supply ? formatLargeNumber(md.max_supply) : '∞'}</span>
          </div>
          <div className="coin-detail-stat">
        ...
        <div className="coin-detail-section">
          ...
          <div className="coin-detail-sentiment">
            <div className="coin-detail-sentiment-bar">
              <div className="coin-detail-sentiment-up" style={{ width: `${sentimentUp}%` }}>
                {sentimentUp > 15 && <span>{sentimentUp.toFixed(0)}%</span>}
              </div>
              <div className="coin-detail-sentiment-down" style={{ width: `${sentimentDown}%` }}>
        ...
        <div className="coin-detail-section">
          ...
          <div className="coin-detail-links">
            ...
            {explorers.slice(0, 3).map((url, i) => {
              const label = url.includes('etherscan')
                ? 'Etherscan'
