...
import { fetchXStocks, fetchIndicesData, fetchIndicesEtfData } from '../core/api.js';
...
export default function BoursePage() {
  ...
  useEffect(() => {
    ...
    fetchIndicesEtfData()
      ...
      .catch((e) => console.warn('Indices ETF error:', e));
  ...
  const renderIndexCard = (idx) => {
    const isEtf = Boolean(idx.isin);
    const holding = isEtf ? holdings[idx.id] || {} : null;
    ...
    return (
      <div key={idx.id} className={`index-card ${isEtf ? gainClass : ''}`}>
        <div className="index-name">{idx.name}</div>
        {idx.isin && <div className="index-isin">{idx.isin}</div>}
        <div className="index-price">{currPrice != null ? `${currPrice.toFixed(2)} ${currency}` : 'N/A'}</div>
        {idx.change != null && (
        ...
        {!isEtf ? (
          <IndexChart symbol={idx.symbol} currency={idx.currency} />
        ) : (
          <>
            ...
            {invested != null && (
              <div className="holding-summary">
                ...
                <div className={`holding-row gain-row ${gainClass}`}>
                  ...
                  <span>
                    {gainTotal >= 0 ? '+' : ''}
                    {formatCurrency(Math.abs(gainTotal))} {currency}
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          {indices && <div className="indices-grid">{indices.map(renderIndexCard)}</div>}

          {etfs && (
            <>
              <div className="section-divider" />
              <div className="indices-grid">{etfs.map(renderIndexCard)}</div>
            </>
          )}
